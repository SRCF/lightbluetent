...
def update(uid, update_type):
    ...
    if update_type == "room_details":
        # grab room details from the POST data
        keys = ("group_name", "website", "description", "short_description")
        ...
        if "logo" in request.files:
            ...
            if logo.content_length > 0:
                logo_filename, logo_extension = os.path.splitext(logo.filename)
                images_dir = current_app.config["IMAGES_DIR"]

                has_valid_extension = (
                    logo_extension in current_app.config["LOGO_ALLOWED_EXTENSIONS"]
                )
                has_valid_name = logo_filename != ""
                # request.files contains a dict of empty file streams, even if you don't upload anything
        ...
        # grab room details from the POST data
        keys = ("new_session_day", "new_session_start", "new_session_end")
        values = get_form_values(request, keys)
        ...
        if values["new_session_start"] and values["new_session_end"]:
            ...
            if t1 > t2:
                ...
                # we need this to ensure that sqlalchemy updates the object
        ...
        if "bbb_logo" in request.files:
            bbb_logo = request.files["bbb_logo"]

            # see logo for why we check this
            if bbb_logo.content_length > 0:
                bbb_logo_filename, bbb_logo_extension = os.path.splitext(
                    bbb_logo.filename
                )
                images_dir = current_app.config["IMAGES_DIR"]

                has_valid_extension = (
                    bbb_logo_extension in current_app.config["LOGO_ALLOWED_EXTENSIONS"]
                )
                has_valid_name = bbb_logo and bbb_logo_filename != ""

                if has_valid_extension and has_valid_name:
                    # Delete the old logo if it's not the default
...
def reset_banner(uid):
    ...
    if group.banner_text is not None:
