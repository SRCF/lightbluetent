'''
    ...
    This is based on commit 683fc4a of
    https://github.com/maxcountryman/flask-seasurf , which is master at the
    time of writing. The version on PyPI is somewhat old, and doesn't work with
    our version of flask. In particular, our version of flask does not support
    the samesite attribute on cookies.

    The only change made (apart from this comment) is that we commented out the
    response.set_cookie line in _set_csrf_token, since samesite attribute is
    not supported and the token is already set in the session. I'm not sure if
    this line is useful but it functions well without it.
'''
